<?php
/**
 * HTML to Gutenberg Blocks converter for Import Eventbrite Events.
 *
...
class Import_Eventbrite_Events_Html_To_Blocks {

	/**
	 * Convert HTML string to Gutenberg block markup.
	 *
	 * @since  1.0.0
	 * @param  string $html Raw HTML from Eventbrite API.
	...
	private function quote_block( string $inner ): string {
		...
		// If inner HTML already contains <p> tags (e.g. Eventbrite sends
	...
	private function embed_block( DOMNode $node ): string {
		...
		// Normalize: strip trailing slash Eventbrite sometimes adds.
		$src = rtrim( $src, '/' );
